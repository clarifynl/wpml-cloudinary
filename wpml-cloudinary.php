<?php

/**
 * @since             1.0.0
 * @package           wpml/cloudinary
 *
 * @wordpress-plugin
 * Plugin Name:       WPML Cloudinary
 * Plugin URI:        https://wpml.org/
 * Description:       Multilingual support for the Cloudinary WordPress plugin
 * Version:           1.1.1
 * Author:            Clarify
 * Author URI:        https://clarify.nl
 * License:           GPL-2.0+
 * License URI:       http://www.gnu.org/licenses/gpl-2.0.txt
 * Text Domain:       wpml-cloudinary
 */

// If this file is called directly, abort.
if ( ! defined( 'WPINC' ) ) {
	die;
}

/**
 * Currently plugin version.
 * Start at version 1.0.0 and use SemVer - https://semver.org
 * Rename this for your plugin and update it as you release new versions.
 */
define( 'WPML_CLOUDINARY_VERSION', '1.1.1' );
define( 'WPML_CLOUDINARY_PLUGIN_PATH', __DIR__ );

/**
 * The code that runs during plugin activation.
 * This action is documented in includes/class-wpml-cloudinary-activator.php
 */
function activate_wpml_cloudinary() {
	require_once plugin_dir_path( __FILE__ ) . 'includes/class-wpml-cloudinary-activator.php';
	WPML_Cloudinary_Activator::activate();
}

/**
 * The code that runs during plugin deactivation.
 * This action is documented in includes/class-wpml-cloudinary-deactivator.php
 */
function deactivate_wpml_cloudinary() {
	require_once plugin_dir_path( __FILE__ ) . 'includes/class-wpml-cloudinary-deactivator.php';
	WPML_Cloudinary_Deactivator::deactivate();
}

register_activation_hook( __FILE__, 'activate_wpml_cloudinary' );
register_deactivation_hook( __FILE__, 'deactivate_wpml_cloudinary' );

/**
 * The core plugin class that is used to define internationalization,
 * admin-specific hooks, and public-facing site hooks.
 */
require plugin_dir_path( __FILE__ ) . 'includes/class-wpml-cloudinary.php';

/**
 * Begins execution of the plugin.
 *
 * Since everything within the plugin is registered via hooks,
 * then kicking off the plugin from this point in the file does
 * not affect the page life cycle.
 *
 * @since    1.0.0
 */
function run_wpml_cloudinary() {

	$plugin = new WPML_Cloudinary();
	$plugin->run();

}
run_wpml_cloudinary();
